...
from mlora.checkpoint import CheckpointOffloadFunction, check_backward_validity, detach_variable, get_device_states, pack_hook, set_device_states, unpack_hook
...
class TestCheckpointOffloadFunction(unittest.TestCase):
    ...
    def test_checkpoint_offload(self):
        
        def run_function(a, b):
            return a + b
        a = torch.tensor([1.0, 2.0], requires_grad=True)
        b = torch.tensor([3.0, 4.0], requires_grad=True)
        result = CheckpointOffloadFunction(run_function, a, b)
...
class TestCheckpointRecomputeFunction(unittest.TestCase):
    ...
    def test_checkpoint_recompute(self):
        ...
        a = torch.tensor([1.0, 2.0], requires_grad=True)
        b = torch.tensor([3.0, 4.0], requires_grad=True)
        result = CheckpointRecomputeFunction.apply(run_function, a, b)
        self.assertTrue(torch.equal(result, torch.tensor([4.0, 6.0])))
